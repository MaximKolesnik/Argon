...
public:
	...
	float32 AR_VEC_CALL DeterminantFull(void) const;
	//Assumes the matrix is an affine transformation matrix
	float32 AR_VEC_CALL DeterminantFast(void) const;

	Matrix4 AR_VEC_CALL Transposed(void) const;
	...
	Matrix4 AR_VEC_CALL Inverted(void) const;
	Matrix4& AR_VEC_CALL Inverse(void);
